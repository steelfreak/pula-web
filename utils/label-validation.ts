...
export const labelValidationRules: ValidationRule[] = [
  ...
  },
  {
    ...
    severity: "warning",
    message: "Label should end with a period, exclamation mark, or question mark",
  },
  {
    id: "no-profanity",
    name: "No Profanity",
  ...
  },
  {
    ...
    validate: (label: string) => {
      ...
      return (
        trimmed.includes(" ") &&
        (trimmed.includes("is") || trimmed.includes("are") || trimmed.includes("refers") || trimmed.length > 20)
      )
    },
    severity: "info",
...
export function validateLabel(label: string): ValidationResult {
  ...
  const passedRules = totalRules - errors.length - warnings.length - info.length
